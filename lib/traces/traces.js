...
function indexToolResults(records) {
  /** @type {Map<string, { endMs: number, isError: boolean, content: unknown, agentId: string | null, outputBytes: number, outputTokens: number }>} */
  const byId = new Map();
...
/**
 * First eligible record for a given `requestId` within `[startIdx, endIdx]`,
 * scanned in JSONL order. Used by both the inference emitter (to source
 * `usage` / `model` / `id` from the same row dedupe picks) and any other
 * caller that needs the canonical row of a response.
 *
 * @param {TranscriptRecord[]} records
 ...
 */
function findCanonicalRecord(records, requestId, startIdx, endIdx) {
 ...
 *
 * @param {TranscriptRecord[]} records
 * @param {number} startIdx  — slice lower bound (inclusive)
 * @param {number} endIdx    — slice upper bound (inclusive)
 * @param {string} parentSpanId
 * @returns {{ inferences: SpanNode[], toolUseToInference: Map<string, string> }}
 */
function buildInferenceSpansForSlice(records, startIdx, endIdx, parentSpanId) {
  ...
  for (const requestId of order) {
    ...
    // Timing: span the full response window. startMs walks back to the
    // record immediately preceding the first row of this group, falling
    // back to the slice's first record's timestamp.
    let startMs = 0;
    for (let i = firstIdx - 1; i >= Math.max(0, startIdx); i--) {
      const t = records[i] ? toMs(records[i].timestamp) : 0;
      if (t) { startMs = t; break; }
    }
    if (!startMs) startMs = toMs(records[Math.max(0, startIdx)]?.timestamp) || 0;
    ...
    // Prompt: structural walk over non-assistant records before this
    // response, back to the prior assistant row in the slice (CLAUDE.md §2:
    // structural, not timestamp-based).
    ...
    if (thinkingParts.length > 0) {
      events.push({
        ...
        timeMs: eventTimeMs,
        attributes: {
          'gen_ai.reasoning.content': truncate(thinkingParts.join('\n'), ASSISTANT_MAX),
        },
      });
    }
